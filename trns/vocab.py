...
from trns.utils import read_corpus, pad_sents
...
class VocabEntry(object):
    ...
    @staticmethod
    def from_corpus(corpora, size, freq_cutoff=2):
...
class Vocab(object):
    ...
    """
    def __init__(self, vocabs ): #########src_vocab: VocabEntry, tgt_vocab: VocabEntry):
        """ Init Vocab.
    ...
    def build(src_sents, tgt_sents, vocab_size, freq_cutoff) -> 'Vocab':
        ...
        sents = (src_sents, tgt_sents)  ################
        src_tgt = VocabEntry.from_corpus(sents, vocab_size, freq_cutoff) #################
        return Vocab(src_tgt)  ############################
    ...
    def save(self, file_path):
        ...
        """
        json.dump(dict(vocs_word2id=self.vocs.word2id), open(file_path, 'w'), indent=2)
    ...
    def load(file_path):
        ...
        """    
        entry = json.load(open(file_path, 'r'))
        vocs_word2id = entry['vocs_word2id']

        return Vocab(VocabEntry(vocs_word2id))
    ...
    def __repr__(self):
        ...
        """
        return 'Vocab(vocs %d words)' % (len(self.vocs))
    
def generate_wid2cid():
...
def get_wid2cid():
    wid2cid = json.load(open('trns/wid2cid.json', 'r'))
    return {int(k):v for k,v in wid2cid.items()}
...
if __name__ == '__main__':
    ...
    print('generated vocabulary, vocs %d words' % (len(vocab.vocs)))
