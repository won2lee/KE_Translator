...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]], slang, tlang, mapping=False) -> torch.Tensor:
        ...
        if mapping==1:
            ...
            self.count +=1
            """
            source_padded, source_lengths = self.parallel_encode_new(source, slang, mapping=True ) #slang_is_tlang=True)
    ...
    def parallel_encode_new(self,source, lang, mapping=0): #slang_is_tlang=False):
        ...
        if mapping==1:  #slang_is_tlang:
            ...
            X_dict = torch.tensor([[-w.item() if i%5==k and self.dict_mapping[w.item()]>0 and self.dict_mapping[w.item()]!= w.item()
                             else self.dict_mapping[w.item()] for i,w in enumerate(s)] for s in X],device=self.device)
    ...
    def ek_beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70, tlang:str='en') -> List[Khypothesis]:
        ...
        for i,hy in enumerate(completed_hypotheses):
            snl = len(hy.value)
            rpb = 1.4e-05*snl**2 - 0.0043*snl + 0.82 if slang=='en' else 1.6e-05*snl**2 - 0.0037*snl + 0.84 #repeat penalty base
            rp = 5 * min(len(list(set(hy.value))) / (snl+0.0001) - rpb, 0)
            to_check =  hy.score/(snl+0.0001) + (-2*(snl/rL-1)**2+(snl/rL-1)*0.4 if snl/rL<1 else -0.2*(snl/rL-1)**2+(snl/rL-1)*0.4) + rp

            #to_check =  hy.score/(snl+0.0001) #+ 0.85*min(1-rL/(snl+0.0001),0)
            #to_check =  hy.score/(len(hy.xo)+0.0001) + 0.7*len(list(set(hy.value)))/(len(hy.xo)+0.0001) + min(1-rL/(len(hy.value)+0.0001),0)
            #to_check = 1.0*hy.score/(len(hy.xo)+0.0001) + 0.7*len(list(set(hy.value))) / len(hy.value) + 1.0 * ((len(hy.value)*2/src_len[0])**0.4-1)

            #to_check =  hy.score/(len(hy.xo)+0.0001) + (1.0*len(list(set(hy.value))) / (len(hy.value)+0.0001) + 0.5 * ((len(hy.value)*2/(src_len[0]+0.0001))**0.4-1)) * min(src_len[0]/15,1)
           
            #to_check = hy.score/(len(hy.xo)+0.0001) + 0.3*len(list(set(hy.value))) / len(hy.value) + 0.8 * ( len(hy.value)*2/src_len[0])**0.8
            #1/(len(hy.value)*3/src_len[0])
            
            mean_scores.append(to_check) 
            ...
            if completed_hypotheses[i].score/len(completed_hypotheses[i].xo) > best_score:
                ...
                k_best = i
            """
        print(f"mean_score: {sum(mean_scores)/len(mean_scores)}\n{mean_scores}")
